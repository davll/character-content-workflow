...
// packages/easy-ai-runtime/src/text-schema.ts
var init_text_schema = __esm({
  "packages/easy-ai-runtime/src/text-schema.ts"() {
    "use strict";
  }
});
...
async function readOpenAIResponse(response, apiName = "OpenAI") {
...
// packages/easy-ai-runtime/src/openai/image.ts
var image_exports = {};
__export(image_exports, {
  buildGenerateImageResponse: () => buildGenerateImageResponse,
  buildImageEditFormData: () => buildImageEditFormData,
  generateImageOpenAI: () => generateImageOpenAI,
  listImageModelsOpenAI: () => listImageModelsOpenAI
});
async function listImageModelsOpenAI(provider) {
  const response = await openAIJsonRequest(provider, "/models");
  return (response.data ?? []).map((model) => model.id ?? "").filter((id) => MODEL_FILTER.test(id)).sort();
}
async function generateImageOpenAI(provider, request) {
  const baseParams = {
    model: request.model,
    prompt: request.prompt,
    n: request.options.count || 1,
    size: request.options.size || "auto",
    quality: request.options.quality || "auto",
    moderation: request.options.moderation || "auto"
  };
  let response;
  if (request.images.length > 0) {
    response = await openAIMultipartRequest(
      provider,
      "/images/edits",
      buildImageEditFormData(baseParams, request.images)
    );
  } else {
    response = await openAIJsonRequest(provider, "/images/generations", {
      ...baseParams,
      stream: false
    });
  }
...
function buildGenerateImageResponse(response, apiName = "OpenAI") {
...
function mapOpenAIImagesUsageToGenerateImageUsage(response) {
...
function buildImageEditFormData(params, images) {
...
function detectImageFormat(data) {
  ...
  if (data.subarray(0, 4).equals(Buffer.from("RIFF", "ascii")) && data.subarray(8, 12).equals(Buffer.from("WEBP", "ascii"))) {
    return "webp";
  }
  return "unknown";
}
...
var init_image = __esm({
  "packages/easy-ai-runtime/src/openai/image.ts"() {
    "use strict";
    init_rest();
    MODEL_FILTER = /^gpt-image-/;
  }
});

// packages/easy-ai-runtime/src/xai/image.ts
var image_exports2 = {};
__export(image_exports2, {
  buildXAIImageEditBody: () => buildXAIImageEditBody,
  buildXAIImageGenerationBody: () => buildXAIImageGenerationBody,
  generateImageXAI: () => generateImageXAI,
  listImageModelsXAI: () => listImageModelsXAI
});
async function listImageModelsXAI(provider) {
  const response = await openAIJsonRequest(provider, "/models", void 0, {
    baseUrl: XAI_BASE_URL,
    apiName: "xAI"
  });
  return (response.data ?? []).map((model) => model.id ?? "").filter((id) => XAI_IMAGE_MODEL_FILTER.test(id)).sort();
}
async function generateImageXAI(provider, request) {
  const response = request.images.length > 0 ? await openAIJsonRequest(
    provider,
    "/images/edits",
...
function buildXAIImageGenerationBody(request) {
  return {
    model: request.model,
    prompt: request.prompt,
    n: request.options.count || 1,
...
function buildXAIImageEditBody(request) {
  ...
  const references = request.images.map((image) => ({
    type: "image_url",
...
var XAI_BASE_URL, XAI_IMAGE_MODEL_FILTER, MAX_EDIT_IMAGES;
var init_image2 = __esm({
  "packages/easy-ai-runtime/src/xai/image.ts"() {
    ...
    XAI_IMAGE_MODEL_FILTER = /^grok-imagine-image-/;
    MAX_EDIT_IMAGES = 3;
  }
});
...
// packages/easy-ai-runtime/src/index.ts
init_text_schema();

// packages/easy-ai-runtime/src/image.ts
async function generateImage(provider, request) {
  switch (provider.name.toLowerCase()) {
    case "openai": {
      const { generateImageOpenAI: generateImageOpenAI2 } = await Promise.resolve().then(() => (init_image(), image_exports));
      return generateImageOpenAI2(provider, request);
    }
    case "xai": {
      const { generateImageXAI: generateImageXAI2 } = await Promise.resolve().then(() => (init_image2(), image_exports2));
      return generateImageXAI2(provider, request);
    }
    default:
      throw new Error(`Unsupported provider: ${provider.name}`);
  }
}
async function listImageModels(provider) {
  switch (provider.name.toLowerCase()) {
    case "openai": {
      const { listImageModelsOpenAI: listImageModelsOpenAI2 } = await Promise.resolve().then(() => (init_image(), image_exports));
      return listImageModelsOpenAI2(provider);
    }
    case "xai": {
      const { listImageModelsXAI: listImageModelsXAI2 } = await Promise.resolve().then(() => (init_image2(), image_exports2));
      return listImageModelsXAI2(provider);
    }
    default:
      throw new Error(`Unsupported provider: ${provider.name}`);
  }
...
// packages/generate-image/src/logger.ts
...
// packages/generate-image/src/providers.ts
var DEFAULT_MODELS = {
  openai: "gpt-image-2",
  xai: "grok-imagine-image-quality"
};
var DEFAULT_API_KEY_ENVS = {
  openai: "OPENAI_API_KEY",
  xai: "XAI_API_KEY"
};
function getProvider(provider, apiKey, verbose) {
...
async function listModels(provider, apiKey, verbose) {
  const log = createLogger(verbose);
  const normalized = provider.toLowerCase();
  assertSupportedProvider(normalized);
...
async function resolvePrompt(options, log) {
  if (options.promptText !== void 0) {
  ...
  if (options.promptFilePath) {
    const prompt = (await fs2.readFile(options.promptFilePath, "utf-8")).trim();
    if (!prompt) {
...
function buildDryRunResult(outputPath, count) {
  ...
  return {
    status: "skipped",
    message: outputPaths.length === 1 ? `Dry run: image generation skipped. Requested output: ${outputPaths[0]}` : `Dry run: image generation skipped. Requested outputs:
${outputPaths.map((p) => ` - ${p}`).join("\n")}`,
    outputPaths
  };
}
function checkRequestedOutputConflicts(options, log) {
...
function findOutputConflict(outputPaths, force) {
  if (force) {
    return void 0;
  }
...
function validateGenerationOptions(options) {
  ...
  if (options.count !== void 0 && (!Number.isInteger(options.count) || options.count < 1)) {
    throw new Error("Image count must be a positive integer.");
  }
}
function buildGenerationRequest(model, prompt, images, options) {
  return {
    model,
    prompt,
    images,
...
// packages/generate-image/src/cli.ts
async function main(argv = process.argv) {
  ...
  program2.command("models").description("List available image models for a provider").option("-p, --provider <provider>", "API provider (openai, xai)", "openai").option("--api-key-env <name>", "Environment variable name containing the API key").option("-v, --verbose", "Print verbose output", false).action(async (options) => {
    try {
  ...
  const isCommand = (arg) => ["gen", "models", "help"].includes(arg);
  const args = parseArgv.slice(2);
  if (args.length > 0 && !isCommand(args[0]) && args[0] !== "--help" && args[0] !== "-h") {
    parseArgv.splice(2, 0, "gen");
  }
  await program2.parseAsync(parseArgv);
}
function readApiKeyFromEnv(envName) {
...
// packages/generate-image/src/main.ts
loadDotEnv();
void main();
