---
name: meme-hanshoku
description: Generate a character-consistent meme illustration inspired by the "また、繁殖のためか？" / "又是為了繁殖嗎?" line. Use when the user invokes $meme/hanshoku or asks for one character in this meme scene, using this skill's template image as a composition, expression, mood, and dialogue-placement reference before handing off to illustration-generation.
---

# Meme Hanshoku

Transform the user's `$meme/hanshoku` request into an `illustration-generation` request. This skill owns only the meme-specific prompt rewrite and template reference handoff.

Use `assets/template.png` as the required external reference image.

## Workflow

1. Read the user request and remove the `$meme/hanshoku` trigger.
...
7. Otherwise hand off the downstream prompt and `assets/template.png` to `illustration-generation`.
...
```text
<preserved user request>，角色呈現 selected line 迷因式的吐槽、震驚、質疑反應。構圖參考 template 圖的角色 framing、表情強度、戲劇化氣氛，以及對白框或字幕位置。畫面中加入清楚可讀的 selected language 台詞：「<selected line>」
```

Also include these constraints in the downstream prompt:

- Use the template only for composition, character framing, facial-expression intensity, dramatic mood, and dialogue/text placement.
...
- External reference: `skills/meme-hanshoku/assets/template.png`.
- Template role: `composition/expression/mood/dialogue-placement reference for the hanshoku meme scene`.
...
- the prompt is empty after removing `$meme/hanshoku`
